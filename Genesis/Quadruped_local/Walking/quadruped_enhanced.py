...
class Leg:
    ...
    def get_angles_for_point(self, x: float, height: float) -> Optional[Tuple[float, float]]:
        """
        Calculate joint angles in degrees for a specified point using inverse kinematics.
        This is used for GUI visualization and does not move the motors.
        """
        try:
            ...
            # These formulas from the original code directly compute motor rotations
            hip_motor_rotations = (((1.57 - alpha) * 1) / 6.28) * 10.5
            knee_motor_rotations = ((1.57 - alpha + beta - 1.12) * 4.58)
            
            # Conversion factors (must match the GUI's)
            hip_gear_ratio = 10.5 / 6.28
            knee_gear_ratio = 4.58
            
            # Convert motor rotations to degrees for the GUI visualization
            hip_angle_deg = hip_motor_rotations * 57.2958 / hip_gear_ratio
            knee_angle_deg = knee_motor_rotations * 57.2958 / knee_gear_ratio
            
            return (hip_angle_deg, knee_angle_deg)
            
        except (ValueError, AttributeError):
            # Return None if the point is unreachable or calculation fails
            return None
    ...
    def move_to_point(self, x: float, height: float) -> bool:
        ...
        try:
            ...
            # Convert to motor angles (rotations)
            ...
            # Don't print for CPG to avoid spamming console
            # print(f"Invalid position for move_to_point: {e}")
            return False
